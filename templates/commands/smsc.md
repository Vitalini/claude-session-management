...
3. Name the session. If `$ARGUMENTS` is non-empty, that is the name — pass it through exactly as typed, do not improve it. Otherwise pick one yourself: 1–2 words, e.g. "Turing sample", "PROJ-123", "wallet passes"; never a session id, never a sentence; a ticket's key alone is the name. This is what the user types into `sm "<name>"` to come back.
